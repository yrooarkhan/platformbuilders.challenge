...
class AutenticadorTest {
	...
	private BuildersApiProvider autenticadorProvider;

	@BeforeEach
	void instanciaAutenticadorComMocks() {
		this.autenticadorProvider = Mockito.mock(BuildersApiProvider.class);
		Mockito.doReturn(STRING_VAZIA).when(autenticadorProvider).getConfiguracao();

		this.autenticador = new Autenticador(autenticadorProvider);
	}
